...
Fork provisioning is outside the current scope. A future operator workflow may copy the complete source prefix to the fork repository's numeric-ID prefix while preserving every OID shard path, full OID, and object body.
